import { CandidateApplyFormModal, EmployerApplyFormModal } from "@/components/customComponents/MelaApplyFormModals";
    ...
    >
      <div className="px-8 py-4">
        ...
        <div className="flex place-content-end mt-5">
          {/* Redirect to registration if not logged in else open apply for mela modal */}
          {!user ? (
            <a href="https://public-registration.skillmissionassam.org/register?redirect=https://job-mela.skillmissionassam.org/">
              <Button className="rounded-full text-xs h-8">Apply</Button>
            </a>
          ) : (
            user.type === "employer" ? (
            <EmployerApplyFormModal mela={mela} />
            ) : (
            <CandidateApplyFormModal mela={mela} />
            )
          )}
        </div>
      </div>
